...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        easy_points = orm.TestingQuestionDifficulty.objects.get(key=TestingQuestion.DIFF_EASY).value
        normal_points = orm.TestingQuestionDifficulty.objects.get(key=TestingQuestion.DIFF_NORMAL).value
        advanced_points = orm.TestingQuestionDifficulty.objects.get(key=TestingQuestion.DIFF_ADVANCED).value
        orm.TestingQuestion.objects.filter(difficulty=TestingQuestion.DIFF_EASY)\
            .update(points=easy_points)
        orm.TestingQuestion.objects.filter(difficulty=TestingQuestion.DIFF_NORMAL)\
            .update(points=normal_points)
        orm.TestingQuestion.objects.filter(difficulty=TestingQuestion.DIFF_ADVANCED)\
            .update(points=advanced_points)

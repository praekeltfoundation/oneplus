...
class LearnerViewCourseFilter(admin.SimpleListFilter):
    title = _('Course')
    parameter_name = 'id'
    ...
    def queryset(self, request, queryset):
        if self.value() is None:
            ...
            learners = Participant.objects.filter(classs__course_id=self.value).values("learner")
...
class LearnerViewClassFilter(admin.SimpleListFilter):
    title = _('Class')
    parameter_name = 'cid'
    ...
    def queryset(self, request, queryset):
        if self.value() is None:
            ...
            learners = Participant.objects.filter(classs_id=self.value).values("learner")
            return queryset.filter(id__in=learners)
